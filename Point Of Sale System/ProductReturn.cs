...
namespace Point_Of_Sale_System
{
    public partial class ProductReturn : Form
    {
        ...
        SqlDataReader dr;
        string sqlstr;
        public ProductReturn(string userName)
        {
            InitializeComponent();
        ...
        private void productID()
        {
            try
            {
                con = new SqlConnection("Data Source=.;Initial Catalog=POS;Integrated Security=True");
                con.Open();
                sqlstr = "Select QuantityAvailable, UnitSellingPrice from STOCK where ProductID ='" + sALEDataGridView.CurrentRow.Cells[6].Value + "'";
                cmd = new SqlCommand(sqlstr, con);
                dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    lblQuantityAvailable.Text = dr["QuantityAvailable"].ToString();
                    lblUnitSellingPrice.Text = dr["UnitSellingPrice"].ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        private void salecart()
        {
            try
            {
                con = new SqlConnection("Data Source=.;Initial Catalog=POS;Integrated Security=True");
                con.Open();
                sqlstr = "Select AmountReceived, Balance, TotalAmount from SALESCART where [salesCatID] ='" + sALEDataGridView.CurrentRow.Cells[8].Value + "'";
                cmd = new SqlCommand(sqlstr, con);
                dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    lblAmtReceived.Text = dr["AmountReceived"].ToString();
        ...
        private void load()
        {
            ...
            this.sALETableAdapter.Fill(this.pOSDataSet.SALE);
            // TODO: This line of code loads data into the 'pOSDataSet.PRODUCTRETURN' table. You can move, or remove it, as needed.
            //this.pRODUCTRETURNTableAdapter.Fill(this.pOSDataSet.PRODUCTRETURN);
        ...
        private void cartNoTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            sALEBindingSource.Filter = "salesCatID LIKE '%" + searchstring + "%'";
        ...
        private void save()
        {
            ...
            else
            {
                try
                {
                    ...
                    con = new SqlConnection("Data Source=.;Initial Catalog=POS;Integrated Security=True");
                    con.Open();
                    sqlstr = "Insert into PRODUCTRETURN values (@ProductID, @QuantityReturned, @DateOfReturn,  @ComplaintID, @OtherComplaintID, @UserID)";
                    cmd = new SqlCommand(sqlstr, con);
            ...
            try
            {
                con = new SqlConnection("Data Source=.;Initial Catalog=POS;Integrated Security=True");
                con.Open();
                sqlstr = "UPDATE SALESCART SET AmountReceived = @AmountReceived,  Balance = @Balance, TotalAmount = @TotalAmount  where salesCatID = '" + sALEDataGridView.CurrentRow.Cells[8].Value.ToString() + "'";

                cmd = new SqlCommand(sqlstr, con);
                cmd.Parameters.AddWithValue("@AmountReceived", Convert.ToDecimal(lblUnitSellingPrice.Text.ToString()) * (Convert.ToInt32(sALEDataGridView.CurrentRow.Cells[1].Value.ToString())));
                cmd.Parameters.AddWithValue("@Balance", Convert.ToDecimal(lblTotalAmt.Text.ToString()) - 2 * Convert.ToDecimal(lblUnitSellingPrice.Text) * (Convert.ToInt32(sALEDataGridView.CurrentRow.Cells[1].Value.ToString())));
                cmd.Parameters.AddWithValue("@TotalAmount", Convert.ToDecimal(lblTotalAmt.Text.ToString()) - Convert.ToDecimal(lblUnitSellingPrice.Text) * (Convert.ToInt32(sALEDataGridView.CurrentRow.Cells[1].Value.ToString())));
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@ProductID", Convert.ToInt32(sALEDataGridView.CurrentRow.Cells[6].Value.ToString()));
                cmd.ExecuteNonQuery();
                con.Close();
            }
            catch (Exception ex)
            ...
            //((DataRowView)sALEBindingSource.Current)["salesCatID"] = sALEDataGridView.CurrentRow.Cells[8].Value.ToString();
            //((DataRowView)sALEBindingSource.Current)["ProductID"] = Convert.ToInt32(productIDTextBox.Text.ToString());
            //((DataRowView)sALEBindingSource.Current)["QuantitySold"] = -1 * Convert.ToInt32(QtyReturnednumericUpDown.Value);
            //((DataRowView)sALEBindingSource.Current)["SalePrice"] = -1 * Convert.ToDecimal(lblUnitCostPrice.Text) * ((Convert.ToInt32(sALEDataGridView.CurrentRow.Cells[1].Value.ToString()) - Convert.ToDecimal(QtyReturnednumericUpDown.Text)));
            ...
            {               
                ...
                cmd.Parameters.AddWithValue("@QuantitySold", -1 * Convert.ToInt32(QtyReturnednumericUpDown.Value));
                cmd.Parameters.AddWithValue("@SalePrice", -1 * Convert.ToDecimal(lblUnitSellingPrice.Text) * ((Convert.ToInt32(sALEDataGridView.CurrentRow.Cells[1].Value.ToString()) - Convert.ToDecimal(QtyReturnednumericUpDown.Text))));
                cmd.Parameters.AddWithValue("@SaleDate", Convert.ToDateTime(dateLabel.Text));
                cmd.Parameters.AddWithValue("@UserID", Convert.ToInt32(userIDTextBox.Text));
                cmd.Parameters.AddWithValue("@salesCatID", sALEDataGridView.CurrentRow.Cells[8].Value.ToString());
                cmd.ExecuteNonQuery();
        ...
        {
            product();
            //QtyReturnednumericUpDown.Maximum = Convert.ToDecimal(quantitySoldTextBox.Text);
